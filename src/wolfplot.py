...
class Plot:    
    ...
    def plot_scatter(self, data, x_column_names, y_column_names, data_label = None, fig_kind = "single", regression = False):
        """
            ...
            x_column_names (type: str or tuple): tuple of column names which should be extracted for x-values
            y_column_names (type: str or tuple): tuple of column names which should be extracted for y-values
            data_label(type: tuple): only required if there are multiple x and y columns to label each "layer" of the histogram in the legend or for the titles if small multiples are applied
            fig_kind (type: string): Describes whether data should be plotted on one or on multiple graphs
